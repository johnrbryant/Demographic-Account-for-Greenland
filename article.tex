...
\begin{quote}
Demographic accounts, like any other type of account, are based on the equality of inflows and outflows over a period of time. In the demographic case, the inflows and outflows are composed of human beings who in a given period enter or leave a country...In any period, the equality of inflows and outflows implied that the difference between the open and closing stocks is equal to the net inflow from the outside world.'' \citep[][p. 26]{stone1984accounts}
\end{quote}

The stocks referred to by Richard Stone in the quote above are counts of people at points in time. The flows are counts of events, such as births, deaths, and migrations, during the intervals between the points. A simple example is
\begin{center}
\includegraphics[width=0.4\textwidth]{figures_accounts/fig_account_noage.pdf}
\end{center}
In the demographic system being described, the population contains 16 at time $t$ and 17 people at time $t+1$; during the interval between $t$ and $t+1$, 8 people were born and 7 died. The change in stock (1) equals the net inflow (1), as required.

We may, however, wish to distinguish different groups within the population. The following example classifies people according to their region of residence, with the two regions labelled ``A'' and ``B''.
\begin{center}
\includegraphics[width=0.5\textwidth]{figures_accounts/fig_account_region.pdf}
\end{center}
During the interval between times $t$ and $t+1$ 5 people migrant from region A to region B, and 1 person migrates from region B to region A. For simplicity, we assume there are no births or deaths. We require changes in stocks to equal net inflows separately for each region,
\begin{align}
    11 - 15 & = -5 + 1
    14 - 10 & = -1 + 5.
\end{align}
Equality within each region implies equality at the national level, which in this case reduces to stating that 0 = 0.
...
The change in stock in this case is population at time $t+1$ minus population at time $t$. The net inflow is births minus deaths. Changes in stock equal the net inflow, as required.
